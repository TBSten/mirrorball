...
fun upperCamelCase2KebabCase(string: String): String {
    return string.replace(Regex("([a-z])([A-Z])")) { "${it.groupValues[1]}-${it.groupValues[2]}" }
        .lowercase()
}
...
fun kebabCase2LowerCamelCase(input: String): String =
    input.split("-")
        .mapIndexed { index, word ->
            if (index == 0) word.lowercase() else word.replaceFirstChar {
...
fun autoGenerateFileSpecBuilder(packageName: String, fileName: String): Builder = FileSpec
...
fun autoGenerateFileSpecBuilder(fileName: String) =
    autoGenerateFileSpecBuilder(LibraryPackageName, fileName)
