...
@Composable
fun InputTel(
    attrs: InputTelAttrsScope.() -> Unit = { },
    ref: HtmlTagRef = rememberDefaultHtmlTagRef(),
    dangerouslySetInnerHTML: String? = null,
    content: @Composable InputTelContentScope.() -> Unit = { },
) {
    val attrsScope = InputTelAttrsScope(ref = ref).apply { attrs() }
    val tagContent:
        @Composable()
        (HtmlTagContentScope.() -> Unit) = {
            val scope = InputTelContentScope(ref)
            scope.content()
        }
    HtmlTag(
        localName = "input",
